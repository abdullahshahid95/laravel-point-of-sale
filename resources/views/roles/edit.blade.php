...
<div id="overlay" class=""></div>
                    ...
                    <div class="row">
                        <div class="col-6 pl-5">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    <input type="text" id="name" value="{{ $role->name }}" class="form-control @error('name') is-invalid @enderror" name="name" required>
                                    @error('name')
                            ...
                            <table class="table table-bordered table-hover">
                                ...
                                @foreach ($permissions as $permission)
                                    <tr>
                                        ...
                                        <td>
                                            <input type="checkbox" data-id="{{ $permission->id }}" id="{{ 'view_' . $permission->name }}" value="{{ $role->permissions()->where('permission_id', $permission->id)->first()? $role->permissions()->where('permission_id', $permission->id)->first()->view: 0 }}" @if($role->permissions()->where('permission_id', $permission->id)->first()? $role->permissions()->where('permission_id', $permission->id)->first()->view == 1: 0) checked="true" @endif onclick="onPermissionCheck('{{ 'view_' . $permission->name }}')" class="">
                                        </td>
                                        <td>
                                            <input type="checkbox" data-id="{{ $permission->id }}" id="{{ 'make_' .$permission->name }}" value="{{ $role->permissions()->where('permission_id', $permission->id)->first()? $role->permissions()->where('permission_id', $permission->id)->first()->make: 0 }}" @if($role->permissions()->where('permission_id', $permission->id)->first()? $role->permissions()->where('permission_id', $permission->id)->first()->make == 1: 0) checked="true" @endif onclick="onPermissionCheck('{{ 'make_' . $permission->name }}')" class="">
                                        </td>
                                        <td>
                                            <input type="checkbox" data-id="{{ $permission->id }}" id="{{ 'edit_' .$permission->name }}" value="{{ $role->permissions()->where('permission_id', $permission->id)->first()? $role->permissions()->where('permission_id', $permission->id)->first()->edit: 0 }}" @if($role->permissions()->where('permission_id', $permission->id)->first()? $role->permissions()->where('permission_id', $permission->id)->first()->edit == 1: 0) checked="true" @endif onclick="onPermissionCheck('{{ 'edit_' . $permission->name }}')" class="">
                                        </td>
                                        <td>
                                            <input type="checkbox" data-id="{{ $permission->id }}" id="{{ 'remove_' .$permission->name }}" value="{{ $role->permissions()->where('permission_id', $permission->id)->first()? $role->permissions()->where('permission_id', $permission->id)->first()->remove: 0 }}" @if($role->permissions()->where('permission_id', $permission->id)->first()? $role->permissions()->where('permission_id', $permission->id)->first()->remove == 1: 0) checked="true" @endif onclick="onPermissionCheck('{{ 'remove_' . $permission->name }}')" class="">
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
    ...
    <script type="text/javaScript">
        ...
        function submitForm()
        {
            ...
            $.ajax({
                type: 'PUT',
                url: "{{ url('/role/') }}/" + document.getElementById("roleId").value,
